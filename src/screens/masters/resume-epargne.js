...
export default function ResumeEpargneMasters({navigation,item}) {
...
// api recherche
  ...
  const handleRefresh = () => {
    ...
    getResumeEpargne(); // Appeler la fonction de récupération des données
...
useEffect(()=>{
    getSolde();
    navigation.setOptions({ title: 'Résumé des epargnes' });
...
const intervalId = setInterval(getResumeEpargne2, delay);
// Nettoyer l'intervalle lorsque le composant est démonté ou lorsque l'effet se réexécute
return () => clearInterval(intervalId);
},[])


const getResumeEpargne = async () => {
  setIsLoading(true);
 try {
  const response = await fetch(`https://adores.tech/api/masters/liste-solde-epargne.php`, {
    headers: {
...
const getResumeEpargne2 = async () => {
 ...
 // Solde total epargne
 const getSolde = () =>{
    fetch(`https://adores.tech/api/masters/somme-solde-epargne.php`,{
      method:'post',
        header:{
