...
export default function EpargnesMasters({navigation,route}){
...
const intervalId = setInterval(getEpargnes2, delay);
...
return () => clearInterval(intervalId);
  ...
  const getEpargnes = async () => {
    setIsLoading(true);
   try {
    const response = await fetch(`https://adores.tech/api/data/liste-epargne.php?matricule=${item.matricule}`, {
      headers: {
  ...
  // Solde total epargne
  const getSolde = () =>{
  fetch(`https://adores.tech/api/data/solde-epargne.php?matricule=${item.matricule}`,{
    method:'post',
      header:{
