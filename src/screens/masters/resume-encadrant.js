...
export default function ResumeEncadrantMasters({navigation,item}) {
...
useEffect(()=>{
    getSolde();
    navigation.setOptions({ title: 'Résumé des encadrants' });
...
const getListeUtilisateur = async () => {
  setIsLoading(true);
 try {
  const response = await fetch(`https://adores.tech/api/masters/resume-encadrant.php`, {
    headers: {
...
<View style={styles.overlay}>
  ...
  Solde : {SoldeTotal.length > 0 && !isNaN(parseFloat(SoldeTotal[0].somme))
    ? Math.abs(parseFloat(SoldeTotal[0].somme)).toLocaleString("fr-FR")
    : '0'} F
</Text>
          </View>
        </View>
