...
export default function ListePasswordMasters({navigation}) {
...
useEffect(()=>{
navigation.setOptions({ title: 'Accès non sécurisés' });
...
const getListeUtilisateur = async () => {
  setIsLoading(true);
 try {
  const response = await fetch(`https://adores.tech/api/masters/liste-password.php`, {
    headers: {
...
value={searchTerm}
 />
    ...
    <FlatList
      ...
      renderItem={({item}) => (
        <TouchableOpacity style={styles.experienceItem} onPress={() => navigation.navigate('Details beneficiaire Masters',{item})}>
