...
const NombreStagiaireFiliereMasters = ({navigation,item}) => {
...
const handleRefresh = () => {
  ...
  getNombreStagiaireFiliere(); // Appeler la fonction de récupération des données
  setRefreshing(false); // Indiquer que le rafraîchissement est terminé
};


useEffect(()=>{
    navigation.setOptions({title: 'Liste des filières'});
    getNombreStagiaireFiliere();
},[])

const getNombreStagiaireFiliere = () =>{

  setIsLoading(true);

fetch(`https://adores.tech/api/masters/nombre-stagiaire-filiere.php`,{
  method:'post',
    header:{
...
<View style={{marginTop:10,marginLeft:13,marginRight:15,marginBottom:-5,flexDirection:'row',justifyContent:'space-between'}}>
  ...
  <FlatList
   ...
   data={searchTerm ? searchItems() : data}
    ...
    renderItem={({item}) => (
      <TouchableOpacity style={styles.Card} onPress={() => navigation.navigate('Liste stagiaires par filiere',{item})}>      
...
<View style={{marginRight:25,paddingHorizontal : 15}}>
               ...
               <Text style={{fontSize:18,fontWeight:'700',marginBottom:7,marginRight:25}}>{item.intitule_filiere}</Text>
               <Text style={styles.Categorie} >{item.code_filiere}</Text>
...
export default NombreStagiaireFiliereMasters
