...
export default function   ResumeRelanceMasters({navigation,item}) {
...
useEffect(()=>{
navigation.setOptions({title: 'Résumé des relances'});
...
const getListeInscritsStage = async () => {
  setIsLoading(true);
 try {
  const response = await fetch(`https://adores.tech/api/masters/resume-relance.php`, {
    headers: {
...
value={searchTerm}
 />
    ...
    <FlatList
      ...
      renderItem={({item}) => (
        <TouchableOpacity style={styles.experienceItem} onPress={() => navigation.navigate('Details stagiaires Masters',{item})}>
