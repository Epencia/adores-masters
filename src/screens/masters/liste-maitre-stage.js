...
export default function MaitreStageMasters({navigation,item}){
...
const getMaitreStage = async () => {
  setIsLoading(true);
 try {
  const response = await fetch(`https://adores.tech/api/masters/liste-stagiaire-maitre-stage.php`, {
    headers: {
...
/>
        ...
        <View style={styles.userInfo}>
          ...
          <Text style={styles.userCode}>{item.nombre} stagiaires</Text>
        </View>
      ...
      <View>
        ...
        <TouchableOpacity
          style={styles.followButton} onPress={() => navigation.navigate('Stagiaires par maitre Masters',{item})}>
        ...
        <TouchableOpacity
          ...
          <Text style={styles.followButtonText2}>Appel</Text>
        </TouchableOpacity>
        <View style={{marginBottom:10}}></View>
        <TouchableOpacity
