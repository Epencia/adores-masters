...
export default function StagiaireSansMaitreMasters({navigation,item}) {
...
useEffect(()=>{
navigation.setOptions({title: 'Sans maitre de stage'});
...
const getListeInscritsStage = async () => {
  setIsLoading(true);
 try {
  const response = await fetch(`https://adores.tech/api/masters/liste-stagiaire-sans-maitre.php`, {
    headers: {
...
value={searchTerm}
 />
    ...
    <FlatList
      ...
      renderItem={({item}) => (
        <TouchableOpacity style={styles.userContainer} onPress={() => navigation.navigate('Details stagiaires Masters',{item})}>
