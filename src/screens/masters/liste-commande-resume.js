...
export default function ListeCommandeResume({navigation,item}){
  ...
  const [refreshing, setRefreshing] = useState(false);
    const handleRefresh = () => {
      ...
      getCommandes(); // Appeler la fonction de récupération des données
      setRefreshing(false); // Indiquer que le rafraîchissement est terminé
    };
  
  
  useEffect(()=>{
    navigation.setOptions({title: "Commandes"});
   ...
   const intervalId = setInterval(getCommandes2, delay);
  ...
  const getCommandes = async () => {
    setIsLoading(true);
   try {
    const response = await fetch(`https://adores.tech/api/masters/liste-commande.php`, {
      headers: {
  ...
  return (
    ...
    value={searchTerm}
     />
      ...
      renderItem={({item}) => (
            <TouchableOpacity style={styles.card} onPress={() => navigation.navigate('Details commande',{item})}>
              <View style={[styles.cardIcon, { backgroundColor: "#007BFF" }]}>
                <MaterialCommunityIcons color="white" name='cart' size={20} />
              </View>
