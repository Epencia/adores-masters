...
export default function FormationsMasters({navigation,item}) {
 ...
 const toggleItem = (itemId) => {
  ...
  useEffect(()=>{
navigation.setOptions({title: 'Formations'});
...
return () => clearInterval(intervalId);
  ...
  const getFormations = async () => {
    setIsLoading(true);
   try {
    const response = await fetch(`https://adores.tech/api/masters/liste-formation.php`, {
      headers: {
  ...
  const searchItems = useMemo(() => {
    return () => {
      const filteredData = data.filter(item =>
        ...
        item.NombreCertificat.toString().toLowerCase().includes(searchTerm.toLowerCase())   
      );
      return filteredData;
    };
  }, [data, searchTerm]);
...
/>
      ...
      <View style={styles.videoInfo}>
        ...
        <Text style={styles.videoChannel}>{item.NombreVideo} vidéos</Text>
        <Text style={styles.videoChannel}>{item.NombreCertificat} certificats</Text>
      </View>
      ...
      {expandedItems.includes(item.id_formation) && (
          ...
          <View>
            ...
            <TouchableOpacity
              ...
              <Text style={styles.followButtonText2}>Vidéos</Text>
            </TouchableOpacity>
            <View style={{marginBottom:10}}></View>
            <TouchableOpacity
              style={styles.followButton} onPress={() => navigation.navigate('Certificats Formations Masters',{item})}>
