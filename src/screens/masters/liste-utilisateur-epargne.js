...
export default function ListeUtilisateurEpargne({navigation}) {
...
// api recherche
  ...
  const handleRefresh = () => {
    ...
    getListeUtilisateurEpargne(); // Appeler la fonction de récupération des données
...
useEffect(()=>{
navigation.setOptions({ title: 'Epargnants' });
      // Exécuter la fonction avec cache
const delay = 60000; // Définir le délai à 1 minute
getListeUtilisateurEpargne(); 
// Définir un intervalle pour exécuter la fonction sans cache toutes les 1 minute
const intervalId = setInterval(getListeUtilisateurEpargne2, delay);
// Nettoyer l'intervalle lorsque le composant est démonté ou lorsque l'effet se réexécute
return () => clearInterval(intervalId);
},[])


const getListeUtilisateurEpargne = async () => {
  setIsLoading(true);
 try {
  const response = await fetch(`https://adores.tech/api/masters/liste-utilisateur-epargne.php`, {
    headers: {
...
// liste
const getListeUtilisateurEpargne2 = async () => {
 try {
  const response = await fetch(`https://adores.tech/api/masters/liste-utilisateur-epargne.php`, {
    headers: {
      'Cache-Control': 'no-cache',
...
/>
        ...
        <TouchableOpacity
          style={styles.followButton} onPress={() => navigation.navigate('Liste epargnes utilisateur',{item})}>
