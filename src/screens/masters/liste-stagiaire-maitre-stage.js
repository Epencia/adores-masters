...
export default function   StagiaireParMaitreMasters({navigation,route}) {
...
const getListeInscritsStage = async () => {
  setIsLoading(true);
 try {
  const response = await fetch(`https://adores.tech/api/masters/mes-stagiaires.php?matricule=${item.matricule}`, {
    headers: {
...
<View style={styles.userInfo}>
        ...
        {expandedItems.includes(item.code_stagiaire) && (
          ...
          <View>
            ...
            <TouchableOpacity
              style={styles.followButton} onPress={() => navigation.navigate('Details stagiaires Masters',{item})}>
            ...
            <View style={{marginBottom:10}}></View>
            <TouchableOpacity style={styles.followButton2} onPress={() => navigation.navigate('Details du theme Masters',{item})}>
