...
export default function FiliereParCycleMasters({navigation,route}) {
...
const intervalId = setInterval(getFiliereParCycle2, delay);
...
const getFiliereParCycle = async () => {
  setIsLoading(true);
 try {
  const response = await fetch(`https://adores.tech/api/masters/liste-filiere-cycle.php?cycle=${item.code_cycle}`, {
    headers: {
...
// liste
const getFiliereParCycle2 = async () => {
 try {
  const response = await fetch(`https://adores.tech/api/masters/liste-filiere-cycle.php?cycle=${item.code_cycle}`, {
    headers: {
      'Cache-Control': 'no-cache',
    ...
    <FlatList
      ...
      renderItem={({item}) => (
        <View style={styles.experienceItem}>
          <TouchableOpacity  onPress={() => toggleItem(item.code_filiere)}>
...
/>
        ...
        {expandedItems.includes(item.code_filiere) && (
          
          <View>
            <View style={{marginBottom:20}}></View>
            <TouchableOpacity
              style={styles.followButton} onPress={() => navigation.navigate('Liste des themes filiere',{item})}>
              <Text style={styles.followButtonText}>Thèmes de stage</Text>
            </TouchableOpacity>
            <View style={{marginBottom:10}}></View>
            <TouchableOpacity
              style={styles.followButton2} onPress={() => navigation.navigate('Liste des taches filiere',{item})}>
              <Text style={styles.followButtonText2}>Tâches</Text>
            </TouchableOpacity>
            <View style={{marginBottom:10}}></View>
            <TouchableOpacity
              style={styles.followButton} onPress={() => navigation.navigate('Liste stagiaires par filiere Masters',{item})}>
