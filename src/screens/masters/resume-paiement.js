...
export default function ResumePaiementMasters({navigation,item}) {
...
useEffect(()=>{
    getSolde();
    navigation.setOptions({ title: 'Résumé des paiements' });
...
const getListeUtilisateur = async () => {
  setIsLoading(true);
 try {
  const response = await fetch(`https://adores.tech/api/masters/liste-paiement.php`, {
    headers: {
...
/>
        ...
        <View style={styles.userInfo}>
          ...
          <Text style={styles.userCode}>{parseFloat(item.montant).toLocaleString("fr-FR")} F</Text>
        </View>
        </View>
