...
export default function ListeCycleStatistique({navigation,item}) {
...
const getListeCycle = async () => {
  setIsLoading(true);
 try {
  const response = await fetch(`https://adores.tech/api/masters/liste-cycle-stagiaire.php`, {
    headers: {
...
{data.length > 0 ? (
  ...
  value={searchTerm}
    ...
    <FlatList
      ...
      renderItem={({item}) => (
        <TouchableOpacity style={styles.userContainer} onPress={() => navigation.navigate('Liste stagiaires par cycle Masters',{item})}>
...
/>
        ...
        <View style={styles.userInfo}>
          <Text style={styles.userName}>{item.intitule_cycle} ({item.code_cycle})</Text>
