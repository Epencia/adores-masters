...
export default function ListeTachesFiliere ({navigation,route}) {
...
const getListeTaches = async () => {
  setIsLoading(true);
 try {
  const response = await fetch(`https://adores.tech/api/masters/liste-taches-filiere.php?filiere=${item.code_filiere}`, {
    headers: {
 ...
 />
    ...
    <FlatList
      ...
      renderItem={({item}) => (
        ...
        <Text style={styles.jobTitle}>{item.titre_tache}</Text>
        {expandedItems.includes(item.code_tache) && (
          <View>
            <View style={{marginBottom:10}}></View>
        <HTML source={{ html: item.travail_tache }} contentWidth={windowWidth} ignoredDomTags={ignoredDomTags}/>
