...
export default function ListeFiliereStatistique({navigation,item}) {
...
const handleRefresh = () => {
  ...
  getListeFiliere(); // Appeler la fonction de récupération des données
...
const intervalId = setInterval(getListeFiliere2, delay);
// Nettoyer l'intervalle lorsque le composant est démonté ou lorsque l'effet se réexécute
return () => clearInterval(intervalId);
},[])

// liste
const getListeFiliere = async () => {
  setIsLoading(true);
 try {
  const response = await fetch(`https://adores.tech/api/masters/liste-filiere-stagiaire.php`, {
    headers: {
...
value={searchTerm}
 />
    ...
    <FlatList
      ...
      renderItem={({item}) => (
        <TouchableOpacity style={styles.userContainer} onPress={() => navigation.navigate('Liste stagiaires par filiere Masters',{item})}>
...
/>
        ...
        <View style={styles.userInfo}>
          <Text style={styles.userName}>{item.intitule_filiere} ({item.code_filiere})</Text>
          <Text style={styles.userCode}>{item.nombre}</Text>
