...
export default function CertificatsFormationsMasters({navigation,route}) {
...
// Liste
  const getListeSouscription = async () => {
    setIsLoading(true);
   try {
    const response = await fetch(`https://adores.tech/api/masters/liste-certificat-formation.php?formation=${item.id_formation}`, {
      headers: {
 ...
 />
    ...
    <View style={{marginTop: 25, marginRight:15,marginLeft:15,
      ...
      <FlatList
        ...
        keyExtractor={(item) => item.id_souscription}
        renderItem={({item}) => (
...
/>
      ...
      <View style={styles.videoInfo}>
        <Text style={styles.videoTitle}>{item.nom_prenom}</Text>
        <Text style={styles.videoViews}>{parseFloat(item.prix_souscription).toLocaleString("fr-FR")} F</Text>
        <Text style={styles.videoViews}>{item.titre_formation}</Text>
      </View>
    </View>
