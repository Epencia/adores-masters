...
export default function Formations({navigation,route}) {
  ...
  const getFormations = async () => {
    setIsLoading(true);
   try {
    const response = await fetch(`https://adores.tech/api/data/liste-formation.php?profil=${user[0].profil}&categorie=${item.code}`, {
      headers: {
 ...
 />
    ...
    <View style={{marginTop: 25, marginRight:15,marginLeft:15,
      ...
      <FlatList
        ...
        <TouchableOpacity style={styles.videoContainer} onPress={() => navigation.navigate('Details formation',{item})}>
                       {item.photo64 ? (
            <Image
alt=""
resizeMode="cover"
    ...
    {item.taux > 0 ? (
      <>
        <Text style={{ textDecorationLine: 'line-through' }}>
          {parseFloat(item.prix_formation).toLocaleString("fr-FR")} F   
        </Text>{' '}{' '}    
        <Text style={{ color: 'blue',fontWeight:'bold' }}>
        / {parseFloat(item.PrixCertifie).toLocaleString("fr-FR")} F
        </Text>{' '}
      ...
      parseFloat(item.prix_formation).toLocaleString("fr-FR") + " F"
        ...
        <Text style={styles.videoViews}>{item.diplome}</Text>
      </View>
    </TouchableOpacity>
...
<TouchableOpacity
        ...
        onPress={() => navigation.navigate('Liste des souscriptions')} 
      >
