...
export default function MesRepetiteurs({navigation,item}){
...
const handleRefresh = () => {
  ...
  getListeRepetiteur(); // Appeler la fonction de récupération des données
...
useEffect(()=>{
  navigation.setOptions({title: "Mes répétiteurs"});
  ...
  const intervalId = setInterval(getListeRepetiteur2, delay);
  // Nettoyer l'intervalle lorsque le composant est démonté ou lorsque l'effet se réexécute
  return () => clearInterval(intervalId);
},[])

// liste
const getListeRepetiteur = async () => {
  setIsLoading(true);
 try {
  const response = await fetch(`https://adores.tech/api/data/mes-repetiteurs.php?matricule=${user[0].matricule}`, {
    headers: {
...
// In des mobile money
const getListeRepetiteur2 = async () => {
 try {
  const response = await fetch(`https://adores.tech/api/data/mes-repetiteurs.php?matricule=${user[0].matricule}`, {
    headers: {
      'Cache-Control': 'no-cache',
...
value={searchTerm}
 />
    ...
    <FlatList
      ...
      keyExtractor={(item) => item.code_maitre}
      renderItem={({item}) => (
        <View style={styles.experienceItem}>
         
        <TouchableOpacity  onPress={() => toggleItem(item.code_maitre)}>
...
/>
        ...
        <View style={styles.userInfo}>
          ...
          <Text style={styles.userCode}>Ville : {item.localite} * Niveau : {item.niveau}</Text>
        </View>

        </View>

        
        {expandedItems.includes(item.code_maitre) && (
      ...
      <View>
        ...
        <TouchableOpacity
          ...
          <Text style={styles.followButtonText2}>Supprimer</Text>
        </TouchableOpacity>
      </View>
      )}
...
<View style={styles.overlay}>
        <View
          ...
          <TouchableOpacity style={styles.btn} onPress={() => navigation.navigate('Nouveau repetiteur')} >
          <Text style={styles.btnText}>Deviens répétiteur</Text>
          </TouchableOpacity>
        </View>
      </View>
