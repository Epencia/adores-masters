...
export default function ListeNiveau({navigation,item}) {
  ...
  useEffect(()=>{
    navigation.setOptions({ title: "Niveaux" });
  ...
  const getListePlateforme = async () => {
    setIsLoading(true);
   try {
    const response = await fetch(`https://adores.tech/api/data/liste-niveau.php`, {
      headers: {
 ...
 />
    ...
    <View style={{marginTop: 25, marginRight:15,marginLeft:15,
      ...
      <FlatList
        ...
        keyExtractor={(item) => item.niveau}
        renderItem={({item}) => (

        <TouchableOpacity style={styles.videoContainer} onPress={() => navigation.navigate('Liste repetiteur',{item})}>
                        <View style={[styles.cardIcon, { backgroundColor: "#007BFF" }]}>
                <MaterialCommunityIcons color="white" name='home' size={20} />
              </View>
      

      <View style={styles.videoInfo}>
        <Text style={styles.videoTitle}>{item.niveau}</Text>
        <Text style={styles.videoChannel}>{item.nombre} répétiteurs</Text>
      </View>
    </TouchableOpacity>
